...
# Test or Production, true for test, false for production
TEST = True
...
def compute_psd_de(raw_array, picks, fre):
    ...
    de = differential_entropy(
        raw_array.copy().filter(l_freq=fre[0], h_freq=fre[1], picks=picks).get_data())  # .get_data(picks=picks)
    return psd, de
...
paths_test = ["/1/4_20151111.mat"]

psds = []
des = []

if TEST:
    path_array = paths_test
else:
    path_array = paths

for path in path_array:
    if TEST:
        path = "seedtest/eeg_raw_data" + path
    else:
        path = "seed/eeg_raw_data" + path
